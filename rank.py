...
# Target cities for local scoring
TARGET_CITIES = {"pune", "noida", "delhi", "gurgaon", "ghaziabad", "faridabad"}

# Core ML/AI and Search/Retrieval skills from the JD
CORE_SKILLS = {
...
def is_honeypot(data):
    ...
    # Rule 2: Krutrim or Sarvam AI before 2023 (founded recently)
...
def score_candidate(data):
    ...
    # 8. Behavioral Signals (Add small bonuses, apply strong negative filters)
    ...
    return float(np.round(final_score, 4))

def generate_reasoning(data, rank, score):
    profile = data.get("profile", {})
    career = data.get("career_history", [])
    skills = data.get("skills", [])
    signals = data.get("redrob_signals", {})
    ...
    # Extract notable skills matching core list
    skill_names = [sk.get("name") for sk in skills if sk.get("name") and any(cs in sk.get("name", "").lower() for cs in CORE_SKILLS)]
    top_skills = skill_names[:3]
    if top_skills:
        skills_phrase = f"skills in {', '.join(top_skills)}"
        skills_phrase_short = f"skills in {', '.join(top_skills[:2])}"
    else:
        skills_phrase = "strong ML engineering skills"
        skills_phrase_short = "applied ML systems capability"
        
    # Extract product companies
    prod_jobs = [job.get("company") for job in career if job.get("company", "").strip().lower() in PRODUCT_COMPANIES]
    companies_phrase = f"at {', '.join(prod_jobs[:2])}" if prod_jobs else "at product companies"
    
    notice = signals.get("notice_period_days", 0)
    np_phrase = f"sub-30 day notice ({notice} days)" if notice <= 30 else f"{notice} days notice"
    
    # Check for obvious gaps or concerns
    concerns = []
    if notice >= 90:
        concerns.append(f"notice period is long ({notice} days)")
        
    loc_lower = loc.lower()
    is_local = any(tc in loc_lower for tc in TARGET_CITIES)
    reloc = signals.get("willing_to_relocate", False)
    if not is_local:
        if reloc:
            loc_phrase = f"willing to relocate from {loc.split(',')[0]}"
        else:
            loc_phrase = f"based in {loc.split(',')[0]} (remote preferred)"
            concerns.append("not local and relocation flag is false")
    else:
        loc_phrase = f"locally based in {loc.split(',')[0]}"
        
    sentences = []
    
    if rank <= 15:
        sentences.append(f"Exceptional founding engineer candidate with {exp:.1f} years experience, specializing in {skills_phrase}")
        sentences.append(f"Proven record building ranking/retrieval systems {companies_phrase}")
        sentences.append(f"{loc_phrase} with a {np_phrase}")
    elif rank <= 50:
        sentences.append(f"Strong Senior AI candidate with {exp:.1f} years total experience")
        sentences.append(f"Developed ML/search infrastructure {companies_phrase}")
        sentences.append(f"Demonstrates solid proficiency in {skills_phrase_short}")
        sentences.append(f"{loc_phrase} with {np_phrase}")
    elif rank <= 80:
        sentences.append(f"Experienced engineer ({exp:.1f} years) with focus on backend systems and {skills_phrase}")
        sentences.append(f"Good experience {companies_phrase}")
        sentences.append(f"Active platform engagement and {loc_phrase}")
    else:
        sentences.append(f"Suitable candidate with {exp:.1f} years experience, showing a solid foundation in {skills_phrase}")
        sentences.append(f"Good activity signals on the platform")
        sentences.append(f"{loc_phrase} with {np_phrase}")
        
    if concerns:
        sentences.append(f"Note: {'; '.join(concerns)}")
        
    # Build complete text ensuring correct periods and spacing
    reasoning_text = ". ".join(s.strip() for s in sentences if s)
...
def rank_candidates(candidates_path, out_path):
    ...
    print(f"Candidates remaining after filtering: {len(scored_candidates)}")
    
    # Sort: Primary by score descending, secondary by candidate_id ascending (tiebreaker)
    scored_candidates.sort(key=lambda x: x[0]["candidate_id"])
    ...
    print(f"Successfully ranked top 100 candidates and saved to {out_path}.")
...
def main():
    ...
    print(f"Ranking process completed in {(end_time - start_time).total_seconds():.2f} seconds.")
